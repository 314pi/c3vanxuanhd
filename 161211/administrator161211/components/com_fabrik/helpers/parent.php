...
class FabrikWorker {
	...
	static protected $_audio_mime_types = array(
		...
		{
			$_reservedWords = array("act", "task", "view", "layout", "option", "form_id", "submit", "ul_max_file_size", "ul_file_types", "ul_directory", "tableid", 'rowid', 'itemid', 'fabrik', 'adddropdownvalue', 'adddropdownlabel', 'ul_end_dir');
		...
		function getACLGroups($val, $cond = '>=')
		{
			...
			$signature = serialize($val.$cond);

			if (empty ($aclgroups[$signature])) {
				$db =& JFactory::getDBO();
				//XR:MOD-DROITS
				//$sql = "SELECT name FROM #__core_acl_aro_groups WHERE id $cond '$val' AND id < 28";
				//$db->setQuery($sql);
				//$aclgroups[$signature] = $db->loadResultArray();
				//get lft of the group
				$sql = "SELECT lft FROM #__core_acl_aro_groups WHERE id = '$val' LIMIT 1";
				$db->setQuery($sql);
				$row = $db->loadResultArray();
				//get list lft >= or <= of the group
				if (sizeof($row)==1) {
					$sql = "SELECT name FROM #__core_acl_aro_groups WHERE lft $cond ". $db->Quote($row[0]);
					$db->setQuery($sql);
					$aclgroups[$signature] = $db->loadResultArray();
				} else {
					$aclgroups[$signature] = Array();
				}
			}

			return $aclgroups[$signature];
		}
		...
		/**
		 * determine if J1.5.19 or higher loaded and if mootoolsupgrade plugin is enabled if
		 * so return true, otherwise return false
		 * @return bool
		 */

		public function nativeMootools12()
		{
	...
	/**
	 * enables 2.x code to look more like 3.x code - easier diffs
	 * @return object j db
	 */
	...
	function toObject(&$array, $class = 'stdClass', $recurse = true)
	{
		...
		if (is_array($array)) {
			...
			foreach ($array as $k => $v) {
				// $$$ hugh added sanity check, see
				// http://fabrikar.com/forums/showthread.php?t=21203
				// No valid reason to ever have an empty key'ed index, so ignore it
				// as it will blow up if we (or J!) try and create empty property
				if (!empty($k)) {
