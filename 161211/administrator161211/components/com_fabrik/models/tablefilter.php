...
class FabrikModelTablefilter extends JModel {
	...
	/**
	 * get the table from the tableModel
	...
	/**
	 * $$$ rob activetableid set in content plugin only clear filters on active table (otherwise with n tables in article all qs filters are removed)
	 * @return bol - is the table currently being rendered the table that initially triggered the filter
	 */

	protected function activeTable()
	{
		return JRequest::getInt('id') == JRequest::getInt('activetableid') || JRequest::getVar('activetableid') == '';
	}
	 ...
	 * $$$ rob if the filter should not be applied due to its acl level then set its condition so that it
	 ...
	 from the filter forms field. Can be used in conjunction with a list filter plugin to override a normal fiters option with the
		...
		e.g http://www.epics-ve.eu/index.php?option=com_fabrik&view=table&tableid=5
		*/
	...
	/**
	 * check if the search all field (name=fabrik_table_filter_all) has submitted data
	 *
	...
	private function getSearchAllFilters(&$filters)
	{
		...
		$item =& $this->tableModel->getTable();
		$key = 'com_fabrik.list'.$item->id.'.filter.searchall';

		$search = trim($app->getUserStateFromRequest($key, 'fabrik_table_filter_all'));

		if ($search == '') {
			if (array_key_exists('fabrik_table_filter_all', $_POST)) {
		...
		$tableid = JRequest::getInt('tableid', -1);

		// check that we actually have the correct table id (or -1 if filter from viz)
		if ($this->tableModel->getTable()->id == $tableid || $tableid == -1) {
			if ($this->tableModel->getParams()->get('search-mode-advanced')) {
	...
	private function doBooleanSearch(&$filters, $search)
	{
		...
		$names = $this->tableModel->getSearchAllFields();
	...
	public function clearFilters()
	{
		...
		if (isset($registry->_registry['com_fabrik']['data']->$tid->filter)) {
			...
			// Joomla content plugin e.g. {fabrik view=table id=1 tablename___elementname=foo}
			// these should not be removed when the table filters are cleared
		...
		if ($fromFormId != $this->tableModel->getFormModel()->get('id')) {
	...
	{
		$elements =& $this->tableModel->getElements('id', false);
		...
		foreach ($keys as $elid) {
			...
			if ($key !== false) {
				...
				$filters['label'][$key] = $elparams->get('element_alt_table_heading') == '' ? $element->label : $elparams->get('element_alt_table_heading');
				...
				$filters['required'][] = 0;
				$filters['access'][] = 0;
				//$$$ rob having grouped_to_previous as 1 was barfing this table view for bea, when doing a search all:
				// http://test.xx-factory.de/index.php?option=com_fabrik&view=table&tableid=31&calculations=0&Itemid=16&resetfilters=0
				...
				$filters['label'][] = $elparams->get('element_alt_table_heading') == '' ? $element->label : $elparams->get('element_alt_table_heading');
	...
	private function getSearchFormSearchAllFilters(&$filters)
	{
		...
		$app =& JFactory::getApplication();
		$formModel =& $this->tableModel->getFormModel();
	...
	private function getSearchFormFilters(&$filters)
	{
		...
		if ($fromFormId != $formModel->get('id')) {
			$fromForm =& JModel::getInstance('Form', 'FabrikModel');
			...
			$elements =& $this->tableModel->getElements('id');
			$filter_elements =& $this->tableModel->getElements('filtername');
			$key = 'com_fabrik.searchform.form'.$fromFormId.'.filters';
			$tablename = $db->nameQuote($this->tableModel->getTable()->db_table_name);
			...
			for ($i = 0; $i < count($searchfilters['key']); $i++) {
				...
				if (array_key_exists($elid, $elements)) {
					...
					if (array_key_exists($key, $filter_elements)) {
						...
						$joins =& $this->tableModel->getJoins();
				...
				if (!is_a($elementModel, 'FabrikModelElement') || $found === false) {
				...
				if ($index === false) {
					...
					$filters['label'][$index] = $elparams->get('element_alt_table_heading') == '' ? $element->label : $elparams->get('element_alt_table_heading');
	...
	private function getQuerystringFilters(&$filters)
	{
		...
		$elements =& $this->tableModel->getElements('filtername');
	...
	private function getPostFilterArray()
	{
		if (!isset($this->request)) {
			...
			if (array_key_exists('fabrik___filter', $request) && array_key_exists('table_'.$item->id, $request['fabrik___filter'])) {
				$this->request = $request['fabrik___filter']['table_'.$item->id];
			} else {
				$this->request = array();
			}
		}
	...
	private function getPostFilters(&$filters)
	{
		...
		if (!empty($request) && array_key_exists('key', $request)) {
			...
			// $$$ rob - in default_horiz_search tmpl only filters whose elements are set to show in table are shown
		...
		$this->tableModel->tmpFilters = $filters;
		FabrikHelperHTML::debug($filters, 'filter array: before onGetPostFilter');
		$this->tableModel->getPluginManager()->runPlugins('onGetPostFilter', $this->tableModel, 'table', $filters);
		FabrikHelperHTML::debug($filters, 'filter array: after onGetPostFilter');
		$filters = $this->tableModel->tmpFilters;
	}
	...
	private function getSessionFilters(&$filters)
	{
		...
		$elements =& $this->tableModel->getElements('id');
		$item =& $this->tableModel->getTable();
		$key = 'com_fabrik.table'.$item->id.'.filter';
		...
		$search = $app->getUserStateFromRequest($key, 'fabrik_table_filter_all');
		...
		for ($i = 0; $i < count($sessionfilters['key']); $i++) {
			...
			if ($elid == -1) {
				...
				$elementModel =& $elements[$elid];
				if (!is_a($elementModel, 'FabrikModelElement') && !in_array($elid, $pluginKeys)) {
					continue;
				}
				// check list plugins
				if (in_array($elid, $pluginKeys)) {
					...
					$label = $elparams->get('element_alt_table_heading') == '' ? $element->label : $elparams->get('element_alt_table_heading');
	...
	{
		$pluginManager =& $this->tableModel->getPluginManager();
