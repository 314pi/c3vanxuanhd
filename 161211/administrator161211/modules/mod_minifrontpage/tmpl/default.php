...
if ($cat_title) { 
	$query = "SELECT title FROM #__sections WHERE id =". $row->sectionid; 
	$database->setQuery( $query ); 
	$categ = $database->loadResult(); 
	$catlink   = JRoute::_(ContentHelperRoute::getSectionRoute($row->sectionid));
}
...
if ( $num_intro ) { 
	if ($col==0) { ?><tr><?php }?> 
		...
		if ($show_date) { 
			...
			$date = JHTML::_('date', $date, $params->get( 'date_format' ), $mainframe->getCfg('offset')); 
			echo $date; 
			echo '</span>'; 
		} 
			if($show_author_type) {
		...
		echo '<td valign="top" colspan="'.$columns.'">'; 
		echo '<ul class="minifp">';
		echo "<li class='minifp'>". $title . '<a class="minitime"> ' .$date."</a></li>";  
		$ini_intro = 0; 
		continue; 
	} else { 
		echo "<li class='minifp'>". $title . '<a class="minitime"> ' .$date."</a></li>"; 
